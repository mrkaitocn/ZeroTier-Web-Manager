// public/script.js - Phiên bản cuối cùng, hỗ trợ đầy đủ các tính năng quản lý nâng cao
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateMember = async (networkId, memberId, payload) => {
        ...
        try {
            ...
            // Nếu lỗi, tải lại để khôi phục trạng thái cũ
            if(networkId) await loadMembers(networkId);
            else if(memberElement) memberElement.style.opacity = '1';
        }
    };
    ...
    const loadMembers = async (networkId, isBackground = false) => {
        ...
        try {
            ...
            const focusedMemberId = focusedElement.closest('.list-group-item')?.id;
            const focusedInputClass = focusedElement.className;
